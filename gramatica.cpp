        ...
        for (const auto& kv : g.regras) {
            ...
            for (const auto& prod : kv.second) {//pra cada producao
                for (size_t i = 0; i < prod.size(); ++i) {//para cada simbolo
                    ...
                    if (!beta.empty()) {//FIRST(beta) sem o epsilon ⊆ FOLLOW(B)
                        ...
                        if (firstBeta.count(EPSILON)) {//se tiver epsilon no first beta FOLLOW(A) ⊆ FOLLOW(B)
                        ...
                        for (const auto& s : g.follow[A]) { //B no final, FOLLOW(A) ⊆ FOLLOW(B)
                            if (!g.follow[B].count(s)) {
                                g.follow[B].insert(s);
                                changed = true;
                            }
